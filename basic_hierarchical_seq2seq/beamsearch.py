...
class _Hypothesis(object):
    def __init__(self, sequence, logprob, hists):
        """
    ...
    def extend_k(self, topk, logprobs, hists):
        return [_Hypothesis(self.sequence+[t.item()],
                            self.logprob + lp.item() - i, hists)
...
def next_search_beam(beam, beam_size, finished,
                     end, eoa, topk, lp, hists):
    """generate the next beam(K-best hyps)"""
    topks, lps, hists_list = _unpack_topk(topk, lp, hists)
    hyps_lists = [h.extend_k(topks[i], lps[i], hists_list[i])
                  for i, h in enumerate(beam)]
    hyps = list(concat(hyps_lists))
    finished, beam = _clean_beam(finished, hyps, end, eoa, beam_size)
...
def _unpack_topk(topk, lp, hists):
...
def _clean_beam(finished, beam, end_tok, eoa, beam_size, remove_tri=True):
    ...
    for h in sorted(beam, reverse=True,
        ...
        if remove_tri and _has_repeat_tri(h.sequence):
            h.logprob = -1e9
        if (h.sequence[-1] == end_tok or h.sequence[-1] == eoa):
