...
def lstm_encoder(sequence, lstm,
    ...
    if seq_lens:
        ...
        packed_seq = nn.utils.rnn.pack_padded_sequence(emb_sequence,
                                                       seq_lens)  #此时emb_sequence的size为（81,34,128）,然后根据每句话的长度，把他压缩成两维，比如说
        '''
        a= torch.tensor([[[1,1],[8,8],[1704,1704],[47,47]],
        [[1,1],[4,4],[36,36],[0,0]],
        [[1,1],[4,4],[0,0],[0,0]]])
        a = a.transpose(0, 1)
        length = [4,3,2]
        targets = torch.nn.utils.rnn.pack_padded_sequence(a, length)
        ...
        [1704, 1704],
        [  36,   36],
        [  47,   47]]), batch_sizes=tensor([3, 3, 2, 1]))
        '''
